...
graph TB
  subgraph phone["User's Phone"]
    subgraph browser["Browser (PWA Mode)"]
      wasm["Dioxus WASM<br/>(RSX UI)"]
      sw["Service Worker<br/>(Cache)"]
      tw["Tailwind CSS v4<br/>(Theme)"]
    end
  end

  wasm -->|"HTTP POST<br/>(server functions)"| tunnel

  tunnel["Tailscale Encrypted Tunnel"]

  subgraph docker["Docker Host"]
    subgraph ts["Tailscale Sidecar<br/>(HTTPS:443)"]
      inject["Injects:<br/>Tailscale-User-Login"]
    end
    subgraph server["Life Manager Server"]
      axum["Axum Router"]
      ocr["Tesseract OCR<br/>(chi_tra)"]
      axum --> db["SQLite (r2d2)<br/>/app/data/life_mgr.db"]
    end
    ts -->|proxy| server
  end
...
graph LR
  subgraph "src/"
    models["models/ — Domain layer:<br/>what the data looks like"]
    api["api/ — Application layer:<br/>what you can do with it"]
    server["server/ — Infrastructure layer:<br/>how it's stored and secured"]
    components["components/ — Presentation layer:<br/>reusable UI building blocks"]
    pages["pages/ — Composition layer:<br/>page-level component assembly"]
    route["route.rs — Navigation:<br/>URL ↔ page mapping"]
    main["main.rs — Bootstrap:<br/>entry point, global config"]
  end
```
